...
export default function ProjectGameProfileManager() {
  ...
  const fetchGames = useCallback(async () => {
    try {
      const res = await tcgAdminGet<PaginatedResponse>('/api/tcg/admin/games?pageSize=50');
      setData(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const openEdit = (item: GameItem) => {
    ...
    try {
      links = JSON.parse(item.downloadLinks || '[]') as { label: string; url: string }[];
    } catch {
      links = [];
    }
  ...
  const handleSubmit = async () => {
    try {
      if (editing) {
        await tcgAdminPut(`/api/tcg/admin/games/${editing.id}`, form);
      } else {
        await tcgAdminPost('/api/tcg/admin/games', form);
      }
      setShowForm(false);
      fetchGames();
      setToast({ open: true, message: editing ? '游戏已更新' : '游戏已创建', type: 'success' });
    } catch (err) {
  ...
  const doDelete = async () => {
    try {
      ...
      await tcgAdminDelete(`/api/tcg/admin/games/${id}`);
      fetchGames();
      setToast({ open: true, message: '游戏已删除', type: 'success' });
    } catch (err) {
    ...
    <>
      <div className="card p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-body font-semibold text-text-title">游戏资料管理</h3>
            <p className="text-caption text-text-muted mt-1">维护项目游戏卡片、封面、平台、分类、评价与下载链接等基础资料。</p>
          </div>
        ...
        <div className="grid gap-3">
          {data?.list.map((game) => (
            <div key={game.id} className="card flex items-center gap-4">
              <div className="w-12 h-16 rounded bg-gray-100 flex-shrink-0 overflow-hidden">
                {game.cover ? <img src={game.cover} alt={game.name} className="w-full h-full object-cover" /> : null}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h4 className="text-body font-semibold text-text-title">{game.name}</h4>
          ...
          {(data?.list.length ?? 0) === 0 ? (
            <div className="card py-12 text-center">
              <Star className="w-10 h-10 text-text-disabled mx-auto mb-3" />
              <p className="text-body text-text-muted">暂无游戏资料</p>
              <p className="text-caption text-text-muted mt-1">点击“添加游戏”开始维护项目游戏资料</p>
            </div>
          ) : null}
        </div>
      </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              <div className="flex gap-5">
                <div className="w-36 flex-shrink-0">
                  <ImageUpload
                    ...
                    label="封面"
                    aspect="aspect-[3/4]"
                    uploader={(file, category) => tcgAdminUpload(file, category as 'cards' | 'game')}
                  />
                </div>
                <div className="flex-1 space-y-3">
                  <div>
                    <label className="text-caption font-medium text-text-muted mb-1 block">游戏名</label>
                    <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="输入游戏名称" className="w-full h-9 px-3 rounded-lg border border-border bg-gray-50/50 text-body focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white transition-colors" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-caption font-medium text-text-muted mb-1 block">平台</label>
                      <select value={form.platform} onChange={(e) => setForm({ ...form, platform: e.target.value })} className="w-full h-9 px-3 rounded-lg border border-border bg-gray-50/50 text-body focus:outline-none focus:border-primary transition-colors">
                    ...
                    <div>
                      ...
                      <select value={form.genre} onChange={(e) => setForm({ ...form, genre: e.target.value })} className="w-full h-9 px-3 rounded-lg border border-border bg-gray-50/50 text-body focus:outline-none focus:border-primary transition-colors">
                        <option value="">请选择</option>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {form.downloadLinks.map((link, idx) => (
                    <div key={idx} className="flex items-center gap-2 group/link">
                      ...
                      <input value={link.url} onChange={(e) => {
                        ...
                        setForm({ ...form, downloadLinks: updated });
                      }} placeholder="https://..." className="flex-1 h-9 px-3 rounded-lg border border-border bg-gray-50/50 text-caption focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white transition-colors" />
                  ...
                  <button type="button" onClick={() => setForm({ ...form, downloadLinks: [...form.downloadLinks, { label: '官网', url: '' }] })} className="inline-flex items-center gap-1.5 h-9 px-3 rounded-lg border border-dashed border-border text-caption text-text-muted hover:border-primary hover:text-primary cursor-pointer transition-colors">
      ...
      <ConfirmDialog open={confirmState.open} title="删除游戏" message="确定要删除这款游戏吗？此操作不可撤销。" confirmText="删除" variant="danger" onConfirm={doDelete} onCancel={() => setConfirmState({ open: false, id: '' })} />
